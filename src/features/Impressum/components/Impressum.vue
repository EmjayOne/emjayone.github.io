<template>
  <section id="about" class="mt-3 pt-4 pb-4">
    <div class="container">
      <h2 class="text-highlight-primary mb-3">Legal Notice</h2>
      <p>
        {{ impressum.webmaster.name }}
      </p>
      <p>
        {{ impressum.webmaster.address }}
      </p>
      <p>
        {{ impressum.webmaster.city }}
      </p>
      <p>
        {{ impressum.webmaster.country }}
      </p>
      <p class="mt-1">
        {{ impressum.webmaster.contact.phone }}
      </p>
      <p class="mt-1">
        {{ impressum.webmaster.contact.mail }}
      </p>

      <h3 class="mt-3 mb-2">{{ impressum.dsgvo.title }}</h3>
      <ol type="I">
        <template v-for="item in impressum.dsgvo.ol" :key="item">
          ...
          <ol v-if="item.romanBig.details" type="1">
            <template v-for="(roman, idRoman) in item.romanBig.details">
                ...
                <template v-for="(number, idNumber) in roman.number.details">
                  ...
                  >
                    ...
                    >
                      {{ letter }}
                      <div class="pt-1 pb-1">
                        <p>{{ impressum.hoster.name }}</p>
                        <p>{{ impressum.hoster.address }}</p>
                        <p>{{ impressum.hoster.city }}</p>
                        <p>{{ impressum.hoster.country }}</p>
                        <p>{{ impressum.hoster.contact.mail }}</p>
                      </div>
                    </p>
                    <p
                      v-else-if="
              ...
              >
                <p v-if="idx === 0">
                  {{ number }} {{ impressum.webmaster.name }},
                  {{ impressum.webmaster.address }},
                  {{ impressum.webmaster.city }},
                  {{ impressum.webmaster.country }}
                  ({{ impressum.webmaster.contact.mail }}).
                ...
                <p v-if="idx === 1">
                  {{ number }} {{ impressum.webmaster.contact.mail }}.
                </p>
                <p v-if="idx === 2">
                  {{ number }} {{ impressum.webmaster.address }},
                  {{ impressum.webmaster.city }},
                  {{ impressum.webmaster.country }}.
                </p>
              </template>
